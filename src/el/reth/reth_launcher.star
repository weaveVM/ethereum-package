...
def get_config(
    ...
    if (
        launcher.builder_type == constants.FLASHBOTS_MEV_TYPE or
        launcher.builder_type == constants.COMMIT_BOOST_MEV_TYPE
    ):
    ...
    cmd.extend(
        [
            ...
            "--chain={0}".format(
                launcher.network if launcher.network in constants.PUBLIC_NETWORKS else constants.GENESIS_CONFIG_MOUNT_PATH_ON_CONTAINER + "/genesis.json",
            ),
            "--http",
            ...
            "--http.api=admin,net,eth,web3,debug,txpool,trace{0}".format(
                ",flashbots" if launcher.builder_type == constants.FLASHBOTS_MEV_TYPE or
                                launcher.builder_type == constants.COMMIT_BOOST_MEV_TYPE else "",
            ),
            "--ws",
            "--ws.addr=0.0.0.0",
    ...
    if launcher.network == constants.NETWORK_NAME.kurtosis:
        ...
        launcher.network not in constants.PUBLIC_NETWORKS and
        constants.NETWORK_NAME.shadowfork not in launcher.network
    ):
        cmd.append(
            "--bootnodes=" +
    ...
    files = {
        ...
        constants.BQCONFIG_MOUNTPOINT_ON_CLIENTS: launcher.bq_config_file,
        constants.BQCONFIG_KEY_MOUNTPOINT_ON_CLIENTS: launcher.bq_key_file,
    }

    if persistent:
        files[EXECUTION_DATA_DIRPATH_ON_CLIENT_CONTAINER] = Directory(
            persistent_key = "data-{0}".format(service_name),
            size = int(participant.el_volume_size) if int(participant.el_volume_size) > 0 else constants.VOLUME_SIZE[launcher.network][constants.EL_TYPE.reth + "_volume_size"],
        )
    env_vars = {}
    ...
    if launcher.builder_type == constants.MEV_RS_MEV_TYPE:
        ...
        files[flashbots_rbuilder.MEV_BUILDER_MOUNT_DIRPATH_ON_SERVICE] = flashbots_rbuilder.MEV_BUILDER_FILES_ARTIFACT_NAME
        env_vars.update(
            {
